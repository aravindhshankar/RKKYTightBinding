...
path_to_output = '../../Output/NEWBLGnldos/NegEnergies/'
...
def process_r(r_index):
    ...
    savedict = {'omegavals' : omegavals,
                'r' : r,
                'NLDOS' : results,
                'INFO' : '[0,1,2,3] sites of the default BLG model , delta = 5e-3 * omega if omega > 1e-3 else 5e-2 * omega if omega > 5e-5, else 1e-1 * omega with v2 model and correct right hopping matrix'
                }
    return savedict

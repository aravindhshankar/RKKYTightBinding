...
def test_Ginfkx():
	...
	omegavals = (omega,)
	kxvals = np.linspace(-np.pi,np.pi,10000)
	delta = 1e-5
	RECURSIONS = 25
	dimH = 4
	def ana_approx_integrand (ky, kx ,omega):
		return np.imag(omega / ((-omega + 1j*delta)**2 + (vF**2) * (kx ** 2 + ky ** 2)))
	anasol = np.array([quad(lambda ky : ana_approx_integrand(ky,kxval,omega), -10, 10, points = [vF**2*kxval**2 - omega**2])[0] for kxval in kxvals]) 

	def ana_mat (kx, omega):
		block = vF**2 / (  kx**2 * vF**2 - np.pi**2 * omega**2)
		return np.imag(omega * cmath.sqrt(block) / (vF)**2)

	anamatsol = np.array([ana_mat(kxval, omega) for kxval in kxvals])
	print(anasol[:5])
	
	kDOS = np.array([MOMfastrecDOSfull(omegaval,ret_H0(kx),ret_Ty(),RECURSIONS,delta)
					for omegaval in omegavals for kx in kxvals]).reshape((len(omegavals),len(kxvals),dimH,dimH))
	...
	for i, omegaval in enumerate(omegavals):
		col = 'C' + str(i)
		ax.plot(kxvals, kDOS[i,:,0,0], c = col,  label = f'$\\omega$ = {omegaval:.3f}')
		ax.axvline(omegaval/vF, c = col, ls = '--')
		ax.plot(kxvals, anamatsol, c = 'r', label = 'analytic', ls = '--')
	...
	fig.savefig(os.path.join(path_to_figdir, 'testpeakloc.pdf'))
	# ax.legend()

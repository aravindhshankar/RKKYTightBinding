...
def demonstrate_chunk_sizes(array_length=1600, total_workers=100):
    ...
    print(
        f"Ratio (largest:smallest): {chunk_sizes.max()/chunk_sizes.min():.1f}")
    ...
    print(
        f"Middle chunk: {end_time_per_element * chunk_sizes[mid]:.1f} seconds")
...
def main():
    ...
    print(
        f"Processed chunk {task_id}: {start_idx} to {end_idx} (size: {chunk_size}, Total workers: {total_workers})")

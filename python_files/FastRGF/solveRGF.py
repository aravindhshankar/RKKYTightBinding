...
def MOMfastrecGfull(omega, H0, Ty, RECURSIONS=20, delta=0.001, dochecks=False):
    Tydag = Ty.conj().T
    Gfwd, fwdflag = fastrecGfwd(
        omega, H0, Ty, RECURSIONS, delta, dochecks=dochecks)
    Grev, revflag = fastrecGfwd(
        omega, H0, Tydag, RECURSIONS, delta, dochecks=dochecks)
    if (fwdflag and revflag):
        ...
        # notice the relative 1/np.pi in the ldos definition
        G = custinv(itrmdt)
        if dochecks == True and (np.isnan(G).any() or np.isinf(G).any()):
...
def MOMfastrecNLDOSfull(omega, r, kx, H0, Ty, RECURSIONS=20, delta=0.001, dochecks=False):
    ...
    if (fwdflag and revflag):
        ...
        if dochecks == True and (np.isnan(itrmdt).any() or np.isinf(itrmdt).any()):
            ...
            return np.zeros_like(H0, dtype=np.double)
        G = custinv(itrmdt)
        # This is with the correct convention of fourier transform
        NLDOS = (-0.5/np.pi) * (np.real(G) *
                                np.sin(kx*r) + np.imag(G)*np.cos(kx*r))

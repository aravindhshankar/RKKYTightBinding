...
def fastrecGfwd(omega,H0,Ty,RECURSIONS=20,delta=mpm.mpf('0.001')):
	...
	IMPORTANT : pass omega, delta also as mpm numbers created with strings
	'''
...
def MOMfastrecGfull(omega,H0,Ty,RECURSIONS=20,delta=mpm.mpf('0.001')):
	'''
...
def MOMfastrecDOSfull(omega,H0,Ty,RECURSIONS=20,delta=mpm.mpf('0.001')):
	'''
	...
	DOS = (-1./mpm.pi) * (((Grev)**-1 - Ty*Gfwd*Tydag)**-1).apply(mpm.im)
